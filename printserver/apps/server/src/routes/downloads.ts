    ...
    fastify.get('/downloads/printer-bat/:slug', async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        try {
            ...
            const bat = [
                ...
                `powershell -NoProfile -Command "try { Add-Printer -Name '!PRINTER_NAME!' -DriverName '!DRIVER!' -PortName '!PORT_NAME!' -ErrorAction Stop; exit 0 } catch { Write-Error $_.Exception.Message; exit 1 }" 2>%TEMP%\\\\ps_addprn_err.txt`,
                'if !errorLevel! == 0 (',
                '    echo        [OK] Printer terpasang.',
                ') else (',
                ...
                '    echo  Solusi: install driver printer dulu, lalu jalankan ulang .bat ini.',
                '    echo.',
                '    pause',
                '    exit /b 1',
                ')',
                ...
                '    echo  ============================================================',
                '    echo.',
                '    echo  SETUP MANUAL (jika perlu):',
                '    echo.',
                '    echo  1. Paper Size:',
                '    echo     Settings ^> Devices ^> Printers',
                '    echo     ^> kanan klik "!PRINTER_NAME!" ^> Printing Preferences',
                '    echo     ^> Paper Size ^> Custom: 60mm x 50mm',
                '    echo.',
                '    echo  2. Matikan Bidirectional (hilangkan balon error):',
                '    echo     Settings ^> Devices ^> Printers',
                '    echo     ^> kanan klik "!PRINTER_NAME!" ^> Printer Properties',
                '    echo     ^> tab General ^> tab Bidirectional Communication ^> OFF',
                '    echo.',
                '    echo  3. ZPL Label Size (untuk printer Zebra/ZDesigner):',
                '    echo     Buka printer properties ^> tab Device Settings',
                '    echo     ^> Paper Size: Custom 60mm x 50mm',
                '    echo  ============================================================',
                ') else (',
                ...
                '    echo   - Driver "!DRIVER!" tidak tersedia di PC ini',
